...
import { useMentorNotifications } from '@/hooks/use-mentor-api';
import { motion, AnimatePresence } from 'framer-motion';
import { cn } from '@/lib/utils';
import {
  LayoutDashboard, BookOpen, MessageSquare,
  LogOut, ChevronLeft, ChevronRight,
  Bell, Search, GraduationCap, Menu
} from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { label: 'Programs', icon: BookOpen, path: '/mentor/programs' },
...
export default function MentorLayout() {
  ...
  const { data: notifications } = useMentorNotifications();
  const unreadCount = notifications?.filter(n => !n.isRead).length ?? 0;
  const initials = user ? `${user.firstName?.[0] || ''}${user.lastName?.[0] || ''}`.toUpperCase() : 'MT';

  const handleLogout = () => { logout(); navigate('/mentor/login', { replace: true }); };
      ...
      <div className={cn("flex items-center gap-3 px-4 py-5 border-b border-sidebar-border", collapsed && "justify-center px-2")}>
        ...
        {!collapsed && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="overflow-hidden">
            ...
            <p className="text-[10px] text-sidebar-foreground/60 uppercase tracking-widest">Mentor</p>
          </motion.div>
        )}
      </div>
        ...
        {collapsed ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-sidebar-primary-foreground truncate">{user?.firstName} {user?.lastName}</p>
  ...
  return (
    <div className="h-screen flex bg-background overflow-hidden">
      <motion.aside animate={{ width: collapsed ? 64 : 256 }} transition={{ duration: 0.2, ease: 'easeInOut' }}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto">
          <AnimatePresence mode="wait">
            <motion.div key={location.pathname} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="p-4 lg:p-6">
              <Outlet />
            </motion.div>
          </AnimatePresence>
        </main>
