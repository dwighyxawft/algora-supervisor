...
import { useMentor, useMentorWorkSamples } from '@/hooks/use-api';
...
import { ArrowLeft, Mail, Phone, Globe, Github, Linkedin, Star, BookOpen, Code, MessageSquareWarning, Briefcase } from 'lucide-react';
    ...
    <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              ...
              <div className="flex-1 space-y-3">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {mentor.screening && (
                    <Badge variant="outline">Screening: {mentor.screening.status.replace('_', ' ')}</Badge>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  {mentor.portfolioUrl && (
                    <Button variant="outline" size="sm" asChild><a href={mentor.portfolioUrl} target="_blank" rel="noreferrer"><Globe className="h-4 w-4" /></a></Button>
                  )}
                </div>
              </div>
            </div>
      ...
      <Tabs defaultValue="programs">
        <TabsList className="bg-muted/50">
          ...
          <TabsTrigger value="screening" className="gap-1.5"><Star className="h-3.5 w-3.5" /> Screening</TabsTrigger>
              ...
              <CardContent className="p-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{mentor.screening.currentPhase}/4</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs">Assessment</p>
                    <p className={`font-medium ${mentor.screening.assessmentPassed ? 'text-green-400' : 'text-muted-foreground'}`}>
                      {mentor.screening.assessmentPassed ? 'Passed' : 'Pending'}
                      {mentor.screening.finalAssessmentScore != null && ` (${mentor.screening.finalAssessmentScore}%)`}
                    </p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs">Code Interview</p>
                    <p className={`font-medium ${mentor.screening.codeInterviewPassed ? 'text-green-400' : 'text-muted-foreground'}`}>
                      {mentor.screening.codeInterviewPassed ? 'Passed' : 'Pending'}
                    </p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs">QBot</p>
                    <p className={`font-medium ${mentor.screening.qBotPassed ? 'text-green-400' : 'text-muted-foreground'}`}>
                      {mentor.screening.qBotPassed ? 'Passed' : 'Pending'}
                    </p>
                  </div>
                </div>
        ...
        <TabsContent value="complaints" className="mt-4 space-y-3">
          {mentor.contactComplaints && mentor.contactComplaints.length > 0 ? mentor.contactComplaints.map(c => (
            <Card key={c.id} className="glass-card">
              <CardContent className="p-4 flex items-center justify-between">
                ...
                <Badge className={c.status === 'closed' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-primary/10 text-primary border-primary/20'}>
                  {c.status}
                </Badge>
              </CardContent>
            </Card>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {w.link && (
                    <Button variant="outline" size="sm" asChild>
                      <a href={w.link} target="_blank" rel="noreferrer">View</a>
                    </Button>
                  )}
                </div>
              </CardContent>
